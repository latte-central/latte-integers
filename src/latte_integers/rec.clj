    ...
    (assume [y int]
      ...
      (assume [Hzero (= y zero)]
        ...
        (have <b2> (positive (succ y))
              :by (eq/eq-subst (lambda [z int] (positive (succ z)))
                               (eq/eq-sym Hzero)
                               <b1>))
      ...
      (assume [Hneg (negative y)]
        (have <d1> (negative (pred (succ y)))
              :by (eq/eq-subst (lambda [z int] (negative z))
                               (eq/eq-sym (int/pred-of-succ y))
                               Hneg))
        (have <d2> (equal (g (pred (succ y))) (inv-f (g (succ y))))
              :by ((p/and-elim-right (H (succ y))) <d1>))
        (have <d3> (equal (g y) (inv-f (g (succ y))))
              :by (eq/eq-subst (lambda [z int] (equal (g z) (inv-f (g (succ y)))))
                               (int/pred-of-succ y) <d2>))
    ...
    (assume [y int]
      ...
      (assume [Hneg (negative (pred y))]
        ...
        (have <b2> (equal (g y) (f (g (pred y))))
              :by (eq/eq-subst (lambda [z int] (equal (g z) (f (g (pred y)))))
                               (int/succ-of-pred y)
                               <b1>))
